...
//테마 상세 정보
function fncSelectThemeDetailList(themeNo , themeNm){
	Loading();
	$("#no").val(themeNo);
	$("#themeNm").val(themeNm);
...
//종목 상세 정보
function fncSelectStocksInfo(stock_code){
	document.listForm.action = "<c:url value='/stock/naver/selectStockInfo.do'/>?code="+stock_code;
    document.listForm.submit();
}
</script>
</head>
...
<form name="listForm" action="${pageContext.request.contextPath}/stock/naver/selectThemeList.do" method="post">
	...
	<input type="hidden" name="themeNm" id="themeNm" value=""/>
<div class="board">
