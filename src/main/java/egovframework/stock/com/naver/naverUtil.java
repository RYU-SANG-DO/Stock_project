...
//									System.out.println(resultMap);
		...
		/**
	     * 네이버 금융 리서치 정보를 크롤링한다.
	     * @param type (sise:시황, industry:산업, company:종목)
	     * @return List<NaverResearchVO>
	     */
	    public static List<NaverResearchCrawVO> crawlResearch(String type) throws Exception {
	        List<NaverResearchCrawVO> list = new ArrayList<>();
	        String url = "";
	        String researchType = "";
	        ...
	        try {
	            ...
	            // 2. 테이블 행(tr) 추출 (CSS Selector 분석 결과)
	            Elements rows = doc.select("table.type_1 tr");

	            for (Element row : rows) {
	                // 헤더나 구분선 제외 (데이터가 있는 행만 td가 여러개임)
	                Elements cols = row.select("td");
	                if (cols.size() < 4) continue;

	                NaverResearchCrawVO vo = new NaverResearchCrawVO();
	                
	                // 3. 데이터 파싱
	                // 제목 & 링크 (첫 번째 td 안의 a 태그)
	                Element anchor = cols.get(0).selectFirst("a");
	                if (anchor != null) {
	                    vo.setTitle(anchor.text());
	                    vo.setLinkUrl("https://finance.naver.com" + anchor.attr("href"));
	                }
	                
	                // 작성자/증권사 (두 번째 td)
	                vo.setWriter(cols.get(1).text());
	                
	                // 작성일 (네 번째 td - 세 번째는 pdf 아이콘인 경우가 많음 구조 확인 필요)
	                // 보통 네이버 리서치 리스트: 제목 | 증권사 | 파일 | 작성일 | 조회수
	                vo.setRegDate(cols.get(3).text());
	                
	                // 조회수 (다섯 번째 td)
	                String viewCntStr = cols.get(4).text().replace(",", "");
	                vo.setViewCount(Integer.parseInt(viewCntStr));
	                
	                vo.setResearchType(researchType);
	            ...
	            e.printStackTrace();
	            //throw new Exception("네이버 리서치 크롤링 실패: " + e.getMessage());
	        }
